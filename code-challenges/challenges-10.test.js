...
const getNames = (arr) => {
  // Solution code here...
  let names=[];
  let namesArray = arr.map((n,e) =>{
   let name=   n.name;
   names.push(name);
  })
 let splitString= names.map(e=>{
   return  e.split("");
  })
// var reverseArray = splitString.reverse();
let reverseArray = splitString.map(e=>{
 return e.reverse();
})
let joinArray = reverseArray.map(e=>{
 return  e.join("")
...
let findMaleAndFemale = (data) => {
  ...
  let str = charArray.map(item => item['name']).join(' and ');
  return str;
};
